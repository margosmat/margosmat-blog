...
const techs = [
  ...
  { name: "Claude Code", icon: "claude.svg", color: undefined, type: "image" },
  { name: "Codex", icon: "codex.png", color: undefined, type: "image" },
  { name: "Local LLM", icon: FaBrain, color: "#3178C6" },
  {
    name: "Agent Development Kit",
    icon: "agent-development-kit.png",
    color: undefined,
    type: "image",
  },
  { name: "LangGraph", icon: "langgraph.png", color: undefined, type: "image" },
  { name: "LangChain", icon: "langchain.png", color: undefined, type: "image" },
  { name: "LangSmith", icon: "langsmith.png", color: undefined, type: "image" },
];
...
export default function TechCarousel({
  ...
  return (
    <div className="w-full h-full flex flex-col justify-between overflow-hidden py-4">
      <h3 className="text-3xl md:text-4xl font-bold px-8">{title}</h3>
      ...
      <div className="relative w-full overflow-hidden mt-auto">
        ...
        <div className="flex w-max animate-scroll whitespace-nowrap hover:pause mt-4">
          {[...techs, ...techs].map((tech, index) => (
            ...
            <div
              key={`${tech.name}-${index}`}
              className="flex flex-col items-center justify-center flex-shrink-0 w-24 mr-12"
            >
              {tech.type === "image" ? (
                <img
                  src={`/icons/${tech.icon}`}
